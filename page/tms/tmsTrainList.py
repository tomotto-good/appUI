...
class TmsTrainList(Base):
    ...
    def page_add_train(self, trainNumber, driver, phone):
        self.click_ele(addTrainButtonID)
        self.send_key(text=trainNumber, ID=trainNumberID)
        self.send_key(text=driver, ID=driverID)
        self.send_key(text=phone, ID=phoneID)

    # 返回第index条明细状态，默认第一条
    def page_get_train_status(self, index=0):
        return self.get_text(trainStatusID, index=index)

    # 录入起运地/目的地
    def page_send_address_info(self, origin, destination):
        self.click_ele(originID)  #
        self.send_key(text=origin, ID=addressSearchID)
        self.click_ele(ID='com.mj.app.marsreport.test:id/item_des', index=1)
        time.sleep(1)
        self.click_ele(text='确定')
        time.sleep(1)
        self.click_ele(destinationID)  #
        self.send_key(text=destination, ID=addressSearchID)
        self.click_ele(ID='com.mj.app.marsreport.test:id/item_des', index=1)
        time.sleep(1)
        self.click_ele(text='确定')
        time.sleep(1)
