...
class PageHome(Base):
    ...
    def page_click_message_title(self):
        self.click_ele(ID=home.messageTitle)

    # 点击加号
    def page_click_menu(self):
        self.click_ele(ID=home.menuButtonID)

    # 选择文件
    def page_select_file(self):
        self.click_ele(text='文件')
        time.sleep(2)
        try:
            result = self.driver.exists(text='1626849874506.jpg')
            if result:
                self.click_ele(text='1626849874506.jpg')
            else:
                self.click_ele(description="显示根目录")
                self.click_ele(text='图片')
                self.click_ele(text='Pictures')
                self.click_ele(text='1626849874506.jpg')
